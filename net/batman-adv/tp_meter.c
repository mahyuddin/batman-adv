...
/**
 * batadv_backbone_gw_put - decrement the batadv_tp_vars refcounter and possibly
 *  release it
...
static void batadv_tp_start_worker(struct work_struct *work)
{
	struct batadv_tp_vars *tp_vars;
	struct task_struct *kthread;

	tp_vars = container_of(work, struct batadv_tp_vars, start_work);
		     ...
		     u32 test_length)
{
	 ...
	 */
	INIT_WORK(&tp_vars->start_work, batadv_tp_start_worker);
	queue_work(batadv_event_workqueue, &tp_vars->start_work);
}
